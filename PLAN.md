...
- [~] Flutter debug APK rebuilt/installed after transport-mode changes. Further UI smoke testing was blocked because the Pixel was on the lock screen (`Unlock with PIN or fingerprint`); user asked to test now, so retry ADB/UI smoke next. ADB was healthy over mDNS transport after restarting the server. Next: verify typed chat hits `/chat/text` with no `/bootstrap`/LiveKit job, verify mic button alone starts voice, verify mic-off disables local mic, and smoke Explore + You/Profile.
...
- [x] Parallel agent `Popper` completed; Explore + You/Profile fixes integrated in the production checkpoint.
- [x] 2026-08-10 secret hygiene follow-up started after GitHub warning: redact stale legacy Cloud SQL credential text from tracked docs/plan and replace a scanner-looking test admin password with an obvious dummy before next commit/push.
- [x] Secret hygiene validation: focused `rg` scan is clean for the stale legacy DB password/private-key patterns, and `.venv/bin/python -m pytest functions/bootstrap/test_fake_profile_all_screens.py -q` passes (`1/1`) after replacing the scanner-looking dummy admin password fixture.
...
|---|---|---|---|
| ~~Critical~~ FIXED 2026-07-21 | `functions/bootstrap/main.py` `getconn()` | Hardcoded plaintext legacy Cloud SQL password was previously present in source and later repeated in notes. Found while investigating a stray "Mock PostgreSQL connected for testing" log line. | Fixed in runtime: Cloud SQL code/env dependency was removed during the Firestore migration. Fixed in docs 2026-08-10: stale plaintext credential text redacted from tracked notes. Still recommended: rotate the historical Cloud SQL password in GCP/Secret Manager if that password was ever active, since deleting source/docs does not undo git-history exposure. Moot once the stopped Cloud SQL instance is fully deleted. |
|---|---|---|---|
